...
const COL_MAP: Record<string, string> = {
  ...
  "no factura": "no_factura",
  factura: "no_factura",
  "numero factura": "no_factura",
  "número factura": "no_factura",
  valor: "valor",
  "valor pagado": "valor",
  "valor total": "valor",
  monto: "valor",
...
export async function POST(request: NextRequest) {
  ...
  const sheetName =
    workbook.SheetNames.find(n => norm(n).includes("pago")) ??
  ...
  const DATE_FIELDS = new Set(["fecha_pago"]);
  ...
  await insertClient.from("pagos").delete().neq("id", 0);

  let inserted = 0, errors = 0, primerError = "";
  const BATCH = 200;
  for (let i = 0; i < registros.length; i += BATCH) {
    const { error } = await insertClient.from("pagos").insert(registros.slice(i, i + BATCH));
